...
$$ attchg_i = \beta_1 x_{t,i} + \beta_2 x_{c,i} + \epsilon_i$$

* Describe the ideal Preceptor Table.

In an ideal Preceptor Table we would know the outcome for treatment and control for each individual. 

* Write a few sentences about whether or not our data will allow us to answer this question.

To an extent. It could predict what his general attitude would be over time. 
...
```{r}
fit_att_chg <- stan_glm(att_chg ~ treatment - 1, 
                       data = week_8,
                       family = gaussian(),
                       refresh = 0)
...
fit_att_chg %>%
  ...
  geom_density(fill = "blue", color = "white", alpha = 0.5) +
  labs(title = "Posterior Density Distribution",
         subtitle = 
...
```{r}
new_obs <- tibble(treatment = factor("Treated", 
...
pp <- posterior_predict(fit_att_chg, 
                        newdata = new_obs) %>%
  as_tibble() %>% 
